...
console.log('Data sent:', $form.serialize());
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        if (res.item_quantity <= 0 || res.item_status !== 'active' ) {
...
$(function () {
 ...
 $.ajax({
  ...
  success: function (res) {
    ...
    if (res.success) {
      $card.find('.extra-form-container').html(res.form_html);
    } else {
      alert(res.message);
    }
  },
  ...
  $(document).on('submit', '.new-extra-form', function (e) {
    e.preventDefault();
    const $form = $(this);

    $.ajax({
      url: 'save_extra_item.php',  
      type: 'POST',
      data: $form.serialize(),
      dataType: 'json',
      success: function (res) {
        console.log('Extra form success:', res);
        if (res.success) {
          alert('New extra item added successfully!');
          $form.trigger('reset'); 
        } else {
          alert(res.message || 'Failed to add extra item.');
        }
      },
      error: function (xhr, status, error) {
        console.error('AJAX error:', error, xhr.responseText);
        alert('An error occurred while saving the extra item.');
      }
    });
  });
