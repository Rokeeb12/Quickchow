...
if (!empty($_POST['nub'])) {
    ...
    $stmtStatus->bind_param("si", $deal, $bo);
    $stmtStatus->execute();
...
$status=$kosRow['status'];
 ...
 $status2='pending';
    ...
    LIMIT 1");
$stmt2->bind_param("ssss", $status, $deal,$name,$bo);
$stmt2->execute();
$stmt2->close();
    ...
    $q3= $con->prepare("SELECT SUM(quantity) AS item_qtyy FROM pak WHERE cod = ? AND num = ? AND item=? ");
        $q3->bind_param("sss", $deal, $bo,$name);
        $q3->execute();
        ...
        $stmtUpdate->bind_param("iss", $total, $bo, $deal);
        $stmtUpdate->execute();
        $stmtUpdate->close();
