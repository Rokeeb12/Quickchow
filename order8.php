  ...
  $(document).on('submit', '.sam_form', function (e) {
    ...
    $.ajax({
      ...
      success: function (res) {
        ...
        if (res.success) {
          $('#packContainer').html(res.newbutton);
            ...
            const minusHtml = `
              <form class="">
                ...
                <button type='button' style='border:none; background-color:white; display:block;' >
                        pack${res.new}<strong>(0)</strong>
              </form>`;


              
            $card.find('.minus-containerp').html(minusHtml);
